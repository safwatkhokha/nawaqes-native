...
//
// 🔧 FIX: wrapped in ErrorBoundary to prevent white/black screen on crash.
// NotificationProvider moved INSIDE AppNavigator so a crash in notifications
// doesn't prevent the entire app from rendering.
...
// ─── Error Boundary (catches any JS crash → shows reset button) ─────
...
export default function App() {
  // Hide splash screen as soon as possible (don't wait for JS to fully load)
  React.useEffect(() => {
    const hideSplash = async () => {
      try {
        // Prevent auto-hide first, then hide manually after 100ms
