...
import {
  ...
  SwitchCamera, Mic, MicOff, Loader2, Users,
...
export default function ChannelsScreen({ navigation }: any) {
  ...
  // Floating gifts animation (viewer's own gifts)
  ...
  // Stream gifts ticker (shows recent gifts from all viewers)
    ...
    try {
      ...
      setEngagement(prev => ({
        ...
        [currentStream.id]: {
          ...
          liked: result.liked, likes: result.likes,
        },
      }));
    } catch {}
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEngagement(prev => ({
        ...
        [currentStream.id]: {
          ...((prev[currentStream.id] || { liked: false, saved: false, likes: currentStream.likes, saves: currentStream.saves, comments: currentStream.comments, shares: currentStream.shares })),
          saved: result.saved, saves: result.saves,
        },
      }));
    } catch {}
  };
  ...
  const handleShare = async () => {
    if (!currentStream) return;
    try { await api.shareStream(currentStream.id); } catch {}
    try {
  ...
  const handleFollow = async () => {
    ...
    try {
      if (wasFollowing) await api.unfollowUser(currentStream.hostId);
      else await api.followUser(currentStream.hostId);
    } catch {
      setFollowState(prev => ({ ...prev, [currentStream.hostId]: wasFollowing }));
    }
  };
  ...
  const handleSendGift = async (gift: typeof GIFT_CATALOG[0]) => {
    ...
    try {
      ...
      setStreamGiftsTicker(prev => [...prev, { id: 'self_' + id, icon: gift.icon, name: gift.name, sender: user?.name || 'أنت' }].slice(-5));
  ...
  // ═══ Poll stream gifts ticker (viewer mode) ═══
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (giftsArr.length > 0) setTotalGifts(giftsArr.length);
        const newGifts = giftsArr.filter((g: any) => !seenTickerGiftIds.has(g.id));
        if (newGifts.length > 0) {
          setSeenTickerGiftIds(prev => { const next = new Set(prev); newGifts.forEach(g => next.add(g.id)); return next; });
          const tickerAdditions = newGifts.filter((g: any) => g.sender_id !== user?.id).map((g: any) => ({
            id: g.id, icon: g.gift_icon || '🎁', name: g.gift_name || 'هدية', sender: g.sender_name || 'مشاهد',
          }));
          if (tickerAdditions.length > 0) setStreamGiftsTicker(prev => [...prev, ...tickerAdditions].slice(-5));
        }
      } catch {}
    }, 4000);
    return () => clearInterval(interval);
  ...
  const enterBroadcastMode = async () => {
    ...
    } catch { Alert.alert('خطأ', 'تعذر الوصول للكاميرا'); }
  };

  const exitBroadcastMode = async () => {
    if (isLive && myStream?.id) { try { await api.endStream(myStream.id); } catch {} }
    setBroadcastMode(false); setIsLive(false); setMyStream(null);
    setElapsedTime('00:00'); setViewerCount(0); setHostGifts([]);
    setSeenGiftIds(new Set()); startTimeRef.current = 0; loadFeed();
  };
  ...
  const handleStartStream = async () => {
    ...
    try {
      const stream = await api.startStream({ title: streamTitle.trim() || `بث ${user?.name || ''}` });
      setMyStream(stream); setIsLive(true); startTimeRef.current = Date.now();
    } catch (err: any) { Alert.alert('خطأ', err?.response?.data?.error || 'فشل بدء البث'); }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (newGifts.length > 0) {
          setSeenGiftIds(prev => { const next = new Set(prev); newGifts.forEach(g => next.add(g.id)); return next; });
          newGifts.forEach((g: any, idx: number) => {
            const giftId = Date.now() + Math.random() + idx;
            setTimeout(() => {
              setHostGifts(prev => [...prev, { id: giftId, icon: g.gift_icon || '🎁', name: g.gift_name || 'هدية', amount: g.amount || 0, sender: g.sender_name || 'مشاهد' }]);
              setTimeout(() => setHostGifts(prev => prev.filter(x => x.id !== giftId)), 4000);
            }, idx * 500);
          });
        }
        const chat = await api.getStreamChat(myStream.id).catch(() => []);
        const newMsgs = (chat as any[]).filter((m: any) => !seenGiftIds.has('chat_' + m.id));
        if (newMsgs.length > 0) {
          setSeenGiftIds(prev => { const next = new Set(prev); newMsgs.forEach(m => next.add('chat_' + m.id)); return next; });
          setChatMessages(prev => [...prev, ...newMsgs.map((m: any) => ({ id: m.id, user: m.user || 'مشاهد', text: m.text }))].slice(-10));
        }
      } catch {}
    }, 3000);
    return () => clearInterval(interval);
  ...
  const onScroll = useCallback((e: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    if (idx !== currentIndex && idx >= 0 && idx < streams.length) {
      setCurrentIndex(idx); setShowChat(false);
    }
  ...
  const renderStream = ({ item }: { item: StreamItem }) => {
    ...
    const isFollowing = followState[item.hostId] ?? item.isFollowing;
    ...
    return (
      <View style={styles.streamContainer}>
        {item.videoUrl ? (
          <Image source={{ uri: fixUrl(item.thumbnailUrl || item.videoUrl) }} style={styles.backgroundImage} resizeMode="cover" />
        ) : item.thumbnailUrl ? (
          <Image source={{ uri: fixUrl(item.thumbnailUrl) }} style={styles.backgroundImage} resizeMode="cover" />
        ) : (
        ...
        <View style={styles.topBar}>
          ...
          <View style={styles.liveBadge}>
            ...
            {item.viewers !== undefined && <Text style={styles.viewerText}>· {formatCount(item.viewers)}</Text>}
          </View>
          <View style={{ width: 40 }} />
        </View>
        ...
        <View style={styles.rightBar}>
          <View style={styles.avatarWrap}>
            <Image source={{ uri: fixUrl(item.hostAvatar) || `https://api.dicebear.com/7.x/avataaars/svg?seed=${item.hostId}` }} style={styles.avatar} />
            {!isFollowing && <TouchableOpacity style={styles.followPlusBtn} onPress={handleFollow}><UserPlus color="#fff" size={12} /></TouchableOpacity>}
          </View>
          <TouchableOpacity style={styles.actionBtn} onPress={handleLike}>
            <View style={styles.actionCircle}><Heart color={liked ? '#ef4444' : '#fff'} size={26} fill={liked ? '#ef4444' : 'transparent'} /></View>
            <Text style={styles.actionText}>{formatCount(likes)}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionBtn} onPress={() => setShowChat(true)}>
            <View style={styles.actionCircle}><MessageCircle color="#fff" size={26} /></View>
            <Text style={styles.actionText}>{formatCount(comments)}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionBtn} onPress={handleSave}>
            <View style={styles.actionCircle}><Bookmark color={saved ? '#facc15' : '#fff'} size={26} fill={saved ? '#facc15' : 'transparent'} /></View>
            <Text style={styles.actionText}>{formatCount(saves)}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionBtn} onPress={() => setShowGiftModal(true)}>
            <View style={[styles.actionCircle, { backgroundColor: 'rgba(168,85,247,0.9)' }]}><Gift color="#fff" size={26} /></View>
            <Text style={styles.actionText}>هدية</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionBtn} onPress={handleShare}>
            <View style={styles.actionCircle}><Share2 color="#fff" size={26} /></View>
            <Text style={styles.actionText}>{formatCount(shares)}</Text>
          </TouchableOpacity>
        </View>

        {/* Floating gifts (viewer's own) */}
        {floatingGifts.map(g => (
          <View key={g.id} style={styles.floatingGift}>
            <Text style={styles.floatingGiftIcon}>{g.icon}</Text>
            <View style={styles.floatingGiftLabel}><Text style={styles.floatingGiftText}>{g.name}</Text></View>
          </View>
        ))}
        {/* Floating hearts */}
        {floatingHearts.map(id => <View key={id} style={styles.floatingHeart}><Text style={{ fontSize: 32 }}>❤️</Text></View>)}
        {/* Gifts ticker */}
          ...
          <View style={styles.giftsTicker}>
            {streamGiftsTicker.map(g => (
              <View key={g.id} style={styles.tickerItem}>
                ...
                <Text style={styles.tickerText} numberOfLines={1}><Text style={styles.tickerSender}>{g.sender}</Text> أرسل {g.name}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        <View style={styles.bottomCard}>
          <View style={styles.hostRow}>
            <Image source={{ uri: fixUrl(item.hostAvatar) || `https://api.dicebear.com/7.x/avataaars/svg?seed=${item.hostId}` }} style={styles.hostAvatar} />
            ...
            <TouchableOpacity style={styles.messageBtn} onPress={handleMessage}><Mail color="#fff" size={14} /></TouchableOpacity>
          </View>
    ...
    <View style={styles.container}>
      ...
      {broadcastMode ? (
        <View style={styles.broadcastContainer}>
          {hasCameraPerm && <CameraView ref={cameraRef} style={styles.camera} facing={cameraType} mode="video" mute={!micOn} />}
          <View style={styles.broadcastTopBar}>
            <TouchableOpacity style={styles.broadcastCloseBtn} onPress={exitBroadcastMode}><X color="#fff" size={22} /></TouchableOpacity>
            {isLive ? (
              <View style={styles.broadcastLiveStats}>
                <View style={styles.broadcastLiveBadge}><Radio color="#fff" size={12} /><Text style={styles.broadcastLiveText}>مباشر</Text></View>
                <Text style={styles.broadcastTimer}>{elapsedTime}</Text>
                <View style={styles.broadcastViewerBadge}><Eye color="#fff" size={12} /><Text style={styles.broadcastViewerText}>{viewerCount}</Text></View>
              </View>
            ) : <Text style={styles.broadcastPreviewText}>معاينة الكاميرا</Text>}
          </View>
          <View style={styles.broadcastRightControls}>
            <TouchableOpacity style={styles.broadcastControlBtn} onPress={flipCamera}><SwitchCamera color="#fff" size={22} /></TouchableOpacity>
            <TouchableOpacity style={[styles.broadcastControlBtn, !micOn && styles.broadcastControlBtnDanger]} onPress={toggleMic}>
          ...
          <View style={styles.broadcastBottomBar}>
            <View style={styles.hostRow}>
              <Image source={{ uri: user?.avatar ? fixUrl(user.avatar) : `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.id || 'me'}` }} style={styles.hostAvatar} />
              <View style={styles.hostInfo}>
                <View style={styles.hostNameRow}>
            ...
            {!isLive ? (
              <TextInput style={styles.titleInput} placeholder="عنوان البث (اختياري)..." placeholderTextColor="#94a3b8" value={streamTitle} onChangeText={setStreamTitle} maxLength={100} />
            ) : null}
            {!isLive ? (
              <TouchableOpacity style={[styles.broadcastStartBtn, startingStream && styles.broadcastStartBtnDisabled]} onPress={handleStartStream} disabled={startingStream}>
                {startingStream ? <Loader2 color="#fff" size={20} /> : <><Radio color="#fff" size={20} /><Text style={styles.broadcastStartBtnText}>بدء البث المباشر</Text></>}
              </TouchableOpacity>
            ) : (
              <TouchableOpacity style={styles.broadcastEndBtn} onPress={handleEndStream}><X color="#fff" size={20} /><Text style={styles.broadcastEndBtnText}>إنهاء البث</Text></TouchableOpacity>
            )}
          </View>
        </View>
      ) : (
        ...
        <>
          {streams.length === 0 && !loading ? (
            <View style={styles.emptyState}>
              <View style={styles.emptyIcon}><Radio color="#a855f7" size={48} /></View>
              <Text style={styles.emptyTitle}>لا يوجد بث مباشر الآن</Text>
              <Text style={styles.emptySub}>كن أول من يبدأ البث على نواقص!</Text>
              <TouchableOpacity style={styles.startBtn} onPress={enterBroadcastMode}>
                <Video color="#fff" size={18} /><Text style={styles.startBtnText}>ابدأ بثك المباشر</Text>
              </TouchableOpacity>
            </View>
          ) : loading ? (
            <View style={styles.loadingState}><ActivityIndicator color="#a855f7" size="large" /></View>
          ) : (
            <FlatList data={streams} keyExtractor={(item) => item.id} renderItem={renderStream} pagingEnabled
              showsVerticalScrollIndicator={false} onScroll={onScroll} scrollEventThrottle={16}
              getItemLayout={(_, index) => ({ length: height, offset: height * index, index })} />
          )}
          <TouchableOpacity style={styles.goLiveBtn} onPress={enterBroadcastMode}>
            <Video color="#fff" size={20} /><Text style={styles.goLiveBtnText}>بث مباشر</Text>
          </TouchableOpacity>
        </>
      )}
        ...
        <View style={styles.chatOverlay}>
          <View style={styles.chatPanel}>
            <View style={styles.chatHeader}>
              ...
              <TouchableOpacity onPress={() => setShowChat(false)}><X color="#94a3b8" size={22} /></TouchableOpacity>
            </View>
            <ScrollView style={styles.chatMessages}>
              {chatMessages.length === 0 ? <Text style={styles.chatEmpty}>لا توجد تعليقات بعد — كن أول من يعلق!</Text> :
            ...
            <View style={styles.chatInputBar}>
              <TextInput style={styles.chatInput} placeholder="أضف تعليقاً..." placeholderTextColor="#64748b" value={chatText} onChangeText={setChatText} onSubmitEditing={handleSendChat} />
              <TouchableOpacity style={styles.chatSendBtn} onPress={handleSendChat}><MessageCircle color="#fff" size={18} /></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
          ...
          <View style={styles.giftPanel}>
            <View style={styles.giftHeader}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                ...
                <Text style={styles.giftTitle}>إرسال هدية{currentStream ? ` إلى @${currentStream.hostName}` : ''}</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <View style={styles.walletPill}><Text style={styles.walletText}>رصيدك: {walletBalance ?? '...'} ج.م</Text></View>
                <TouchableOpacity onPress={() => setShowGiftModal(false)}><X color="#94a3b8" size={22} /></TouchableOpacity>
              </View>
            </View>
            <View style={styles.giftGrid}>
              {GIFT_CATALOG.map(gift => {
                ...
                return (
                  <TouchableOpacity key={gift.id} style={[styles.giftItem, !canAfford && styles.giftItemDisabled]} disabled={!canAfford || giftSending} onPress={() => handleSendGift(gift)}>
...
const styles = StyleSheet.create({
  ...
  topBar: { position: 'absolute', top: 50, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, zIndex: 20 },
  topBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' },
  liveBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#ef4444', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16 },
  liveText: { color: '#fff', fontSize: 12, fontWeight: '900' },
  viewerText: { color: 'rgba(255,255,255,0.8)', fontSize: 11, fontWeight: '700' },
  rightBar: { position: 'absolute', right: 12, bottom: 140, alignItems: 'center', gap: 16, zIndex: 20 },
  avatarWrap: { position: 'relative', marginBottom: 8 },
  avatar: { width: 48, height: 48, borderRadius: 24, borderWidth: 2, borderColor: '#fff', backgroundColor: '#334155' },
  followPlusBtn: { position: 'absolute', bottom: -8, left: '50%', transform: [{ translateX: -12 }], width: 24, height: 24, borderRadius: 12, backgroundColor: '#a855f7', alignItems: 'center', justifyContent: 'center' },
  actionBtn: { alignItems: 'center', gap: 4 },
  actionCircle: { width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(0,0,0,0.4)', alignItems: 'center', justifyContent: 'center' },
  actionText: { color: '#fff', fontSize: 11, fontWeight: '700' },
  floatingGift: { position: 'absolute', left: '50%', top: '40%', transform: [{ translateX: -40 }], alignItems: 'center', zIndex: 30 },
  ...
  floatingHeart: { position: 'absolute', right: 70, bottom: 200, zIndex: 25 },
  giftsTicker: { position: 'absolute', top: 100, left: 12, right: 70, zIndex: 18, gap: 4 },
  tickerItem: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(0,0,0,0.6)', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 16, alignSelf: 'flex-start', maxWidth: '100%' },
  tickerIcon: { fontSize: 16 },
  tickerText: { color: '#fff', fontSize: 11, fontWeight: '600' },
  tickerSender: { color: '#fbbf24', fontWeight: '800' },
  totalGiftsBadge: { position: 'absolute', top: 90, left: '50%', transform: [{ translateX: -50 }], flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(251,191,36,0.2)', paddingHorizontal: 12, paddingVertical: 5, borderRadius: 14, zIndex: 17 },
  totalGiftsText: { color: '#fbbf24', fontSize: 11, fontWeight: '800' },
  bottomCard: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, paddingBottom: 24, zIndex: 20, backgroundColor: 'transparent' },
  ...
  followBtn: { backgroundColor: '#a855f7', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 14 },
  followingBtn: { backgroundColor: '#475569' },
  followText: { color: '#fff', fontSize: 11, fontWeight: '900' },
  messageBtn: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#10b981', alignItems: 'center', justifyContent: 'center' },
  streamTitle: { color: '#fff', fontSize: 14, fontWeight: '700', marginBottom: 4 },
  streamDesc: { color: '#cbd5e1', fontSize: 12 },
  emptyState: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 },
  emptyIcon: { width: 96, height: 96, borderRadius: 48, backgroundColor: 'rgba(168,85,247,0.15)', alignItems: 'center', justifyContent: 'center', marginBottom: 24 },
  ...
  camera: { flex: 1 },
  broadcastTopBar: { position: 'absolute', top: 50, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, zIndex: 10 },
  broadcastCloseBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' },
  broadcastPreviewText: { color: '#fff', fontSize: 14, fontWeight: '700' },
  broadcastLiveStats: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  broadcastLiveBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#ef4444', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 14 },
  broadcastLiveText: { color: '#fff', fontSize: 11, fontWeight: '900' },
  broadcastTimer: { color: '#fff', fontSize: 13, fontWeight: '700' },
  broadcastViewerBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(0,0,0,0.5)', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 14 },
  broadcastViewerText: { color: '#fff', fontSize: 11, fontWeight: '700' },
  broadcastRightControls: { position: 'absolute', right: 16, top: '50%', transform: [{ translateY: -50 }], gap: 12, zIndex: 10 },
  broadcastControlBtn: { width: 48, height: 48, borderRadius: 24, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' },
  broadcastControlBtnDanger: { backgroundColor: '#ef4444' },
  broadcastChatOverlay: { position: 'absolute', top: 120, left: 12, right: 80, zIndex: 15, gap: 4 },
  broadcastChatMsg: { flexDirection: 'row', gap: 4, backgroundColor: 'rgba(0,0,0,0.5)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, alignSelf: 'flex-start', flexWrap: 'wrap' },
  broadcastChatUser: { color: '#a855f7', fontSize: 11, fontWeight: '700' },
  broadcastChatText: { color: '#fff', fontSize: 11 },
  broadcastGiftOverlay: { position: 'absolute', top: 200, left: '50%', transform: [{ translateX: -100 }], flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: 'rgba(168,85,247,0.85)', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, zIndex: 50 },
  ...
  broadcastBottomBar: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, paddingBottom: 24, zIndex: 20 },
  titleInput: { backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 10, color: '#fff', fontSize: 14, marginTop: 10, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  broadcastStartBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#ef4444', borderRadius: 14, paddingVertical: 16, marginTop: 12 },
  broadcastStartBtnDisabled: { opacity: 0.6 },
  broadcastStartBtnText: { color: '#fff', fontSize: 16, fontWeight: '900' },
  broadcastEndBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#dc2626', borderRadius: 14, paddingVertical: 16, marginTop: 12 },
  ...
  chatInput: { flex: 1, backgroundColor: '#0f172a', borderRadius: 22, paddingHorizontal: 16, paddingVertical: 10, color: '#fff', fontSize: 14, borderWidth: 1, borderColor: '#334155' },
