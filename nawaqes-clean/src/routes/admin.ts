...
router.delete('/channels/:id', (req: Request, res: Response) => {
  try {
    ...
    db.prepare('DELETE FROM channel_subscribers WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_posts WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_post_reactions WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_post_views WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_comments WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_livestreams WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_livestream_chat WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_livestream_viewers WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_gifts WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_scheduled_streams WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_scheduled_stream_reminders WHERE channel_id = ?').run(req.params.id);
    db.prepare('DELETE FROM channel_live_polls WHERE channel_id = ?').run(req.params.id);
