...
export const CommentsTab: React.FC<CommentsTabProps> = ({ adminComments, setAdminComments, darkMode }) => {
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    try { await adminFetch('DELETE', `/admin/comments/${commentId}`); setAdminComments(prev => prev.filter(c => c.id !== commentId)); toast.success(t('admin.commentDeleted')); } catch { toast.error(t('admin.commentDeleteFailed')); }
  };
