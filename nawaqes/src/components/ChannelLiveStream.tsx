...
export const ChannelLiveStream: React.FC<ChannelLiveStreamProps> = ({
  ...
  const handleStartStream = async () => {
    setStarting(true);
    try {
      // 🔧 FIX: Check if getUserMedia is available first (HTTPS or localhost required)
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        toast.error('البث المباشر يتطلب HTTPS. تأكد من فتح الموقع عبر https://');
        setStarting(false);
        return;
      }
      ...
      try {
        ...
        console.error('[LiveStream] getUserMedia error:', camErr);
        let errMsg = 'تعذّر الوصول للكاميرا/الميكروفون';
        if (camErr?.name === 'NotAllowedError' || camErr?.name === 'PermissionDeniedError') {
          errMsg = 'تم رفض إذن الكاميرا/الميكروفون. يرجى السماح بالوصول من إعدادات المتصفح';
        } else if (camErr?.name === 'NotFoundError' || camErr?.name === 'DevicesNotFoundError') {
          errMsg = 'لم يتم العثور على كاميرا/ميكروفون. تأكد من توصيلها';
        } else if (camErr?.name === 'NotReadableError') {
          errMsg = 'الكاميرا مستخدمة بواسطة تطبيق آخر. أغلق التطبيقات الأخرى';
        } else if (camErr?.message) {
          errMsg = `فشل الوصول للكاميرا: ${camErr.message}`;
        }
        toast.error(errMsg);
        setStarting(false);
        return;
      ...
      if (!mediaStream) {
        toast.error('فشل في الحصول على بث الكاميرا');
        setStarting(false);
        return;
      }
      ...
      // Stop any media stream we got if the API call failed
      if (localStreamRef.current) {
