...
class ApiClient {
  ...
  async startMarketLiveStream(data: { title?: string; description?: string; productName?: string; productPrice?: number; category?: string }) {
    return this.request<any>('/market/market-live/start', { method: 'POST', body: JSON.stringify(data) });
  }
  async endMarketLiveStream(streamId: string, recordingUrl?: string, recordingDuration?: number) {
    return this.request<any>(`/market/market-live/${streamId}/end`, { method: 'POST', body: JSON.stringify({ recordingUrl, recordingDuration }) });
  }
  async getActiveMarketLiveStreams() {
    return this.request<any[]>('/market/market-live/active');
  }
  async joinMarketLiveStream(streamId: string) {
    return this.request<any>(`/market/market-live/${streamId}/viewer-join`, { method: 'POST' });
  }
  async leaveMarketLiveStream(streamId: string) {
    return this.request<any>(`/market/market-live/${streamId}/viewer-leave`, { method: 'POST' });
  }
  async getMarketLiveChat(streamId: string) {
    return this.request<any[]>(`/market/market-live/${streamId}/chat`);
  }
  async sendMarketLiveChat(streamId: string, text: string) {
    return this.request<any>(`/market/market-live/${streamId}/chat`, { method: 'POST', body: JSON.stringify({ text }) });
  }
  async likeMarketLiveStream(streamId: string) {
    return this.request<any>(`/market/market-live/${streamId}/like`, { method: 'POST' });
  }
  async sendLiveStreamGift(streamId: string, giftType: string, giftAmount: number, message?: string) {
    return this.request<any>(`/market/market-live/${streamId}/gift`, { method: 'POST', body: JSON.stringify({ giftType, giftAmount, message }) });
  }
  async getLiveStreamGifts(streamId: string) {
    return this.request<any[]>(`/market/market-live/${streamId}/gifts`);
  }
