...
// ════════════════════════════════════════════════════════════════════
// ─── MARKET LIVE STREAMING (real-time broadcasting) ──────────────────
// ════════════════════════════════════════════════════════════════════

// POST /api/market/market-live/start — Start a live stream
router.post('/market-live/start', authMiddleware, (req: Request, res: Response) => {
  try {
    const payload = (req as any).user as JwtPayload;
    const { title, description, productName, productPrice, category } = req.body;

    // Check if user already has an active stream
    const existing = db.prepare("SELECT id FROM market_live_streams WHERE user_id = ? AND status = 'live'").get(payload.userId);
    if (existing) { res.status(409).json({ error: 'لديك بث مباشر نشط بالفعل' }); return; }

    const id = crypto.randomUUID();
    db.prepare(`INSERT INTO market_live_streams (id, user_id, title, description, product_name, product_price, category)
      VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, payload.userId,
      (title || '').slice(0, 200), (description || '').slice(0, 500),
      (productName || '').slice(0, 200), productPrice || 0, (category || ''));

    const stream = db.prepare(`
      SELECT s.*, u.name as host_name, u.avatar as host_avatar
      FROM market_live_streams s JOIN users u ON s.user_id = u.id
      WHERE s.id = ?
    `).get(id) as any;

    res.json(stream);
  } catch (err: any) {
    res.status(500).json({ error: 'فشل بدء البث', details: err.message });
  }
});

// POST /api/market/market-live/:id/end — End a live stream
router.post('/market-live/:id/end', authMiddleware, (req: Request, res: Response) => {
  try {
    const payload = (req as any).user as JwtPayload;
    const { recordingUrl, recordingDuration } = req.body;

    const stream = db.prepare('SELECT * FROM market_live_streams WHERE id = ?').get(req.params.id) as any;
    if (!stream) { res.status(404).json({ error: 'البث غير موجود' }); return; }
    if (stream.user_id !== payload.userId && !(req as any).user?.isAdmin) {
      res.status(403).json({ error: 'فقط صاحب البث يمكنه إنهاءه' }); return;
    }

    db.prepare("UPDATE market_live_streams SET status = 'ended', ended_at = datetime('now'), recording_url = ?, recording_duration = ? WHERE id = ?")
      .run(recordingUrl || '', recordingDuration || 0, req.params.id);

    // Auto-save as ad_video if recording exists
    if (recordingUrl) {
      try {
        db.prepare(`INSERT INTO ad_videos (id, user_id, video_url, thumbnail_url, duration, status)
          VALUES (?, ?, ?, '', ?, 'active')`)
          .run(crypto.randomUUID(), stream.user_id, recordingUrl, recordingDuration || 0);
      } catch {}
    }

    // Clear viewers
    try { db.prepare('DELETE FROM market_live_viewers WHERE stream_id = ?').run(req.params.id); } catch {}

    res.json({ success: true, message: 'تم إنهاء البث' });
  } catch (err: any) {
    res.status(500).json({ error: 'فشل إنهاء البث' });
  }
});

// GET /api/market/market-live/active — Get active live streams
router.get('/market-live/active', optionalAuth, (req: Request, res: Response) => {
  try {
    const streams = db.prepare(`
      SELECT s.*, u.name as host_name, u.avatar as host_avatar
      FROM market_live_streams s
    ...
    res.status(500).json({ error: 'فشل تحميل البثوث' });
  }
});

// POST /api/market/market-live/:id/viewer-join — Join as viewer
router.post('/market-live/:id/viewer-join', authMiddleware, (req: Request, res: Response) => {
  try {
    const payload = (req as any).user as JwtPayload;
    const streamId = req.params.id;

    // Insert viewer (ignore if already joined)
    db.prepare('INSERT OR IGNORE INTO market_live_viewers (stream_id, user_id) VALUES (?, ?)').run(streamId, payload.userId);

    // Increment viewer count
    db.prepare('UPDATE market_live_streams SET viewer_count = viewer_count + 1, viewer_peak = MAX(viewer_peak, viewer_count + 1), viewer_total = viewer_total + 1 WHERE id = ?').run(streamId);

    const stream = db.prepare('SELECT viewer_count FROM market_live_streams WHERE id = ?').get(streamId) as any;
    res.json({ viewer_count: stream?.viewer_count || 0 });
  } catch {
    res.status(500).json({ error: 'فشل الانضمام' });
  }
});

// POST /api/market/market-live/:id/viewer-leave — Leave as viewer
router.post('/market-live/:id/viewer-leave', authMiddleware, (req: Request, res: Response) => {
  try {
    const payload = (req as any).user as JwtPayload;
    db.prepare('DELETE FROM market_live_viewers WHERE stream_id = ? AND user_id = ?').run(req.params.id, payload.userId);
    db.prepare('UPDATE market_live_streams SET viewer_count = MAX(0, viewer_count - 1) WHERE id = ?').run(req.params.id);
    res.json({ success: true });
  } catch {
    res.status(500).json({ error: 'فشل المغادرة' });
  }
});

// GET /api/market/market-live/:id/chat — Get chat messages
router.get('/market-live/:id/chat', optionalAuth, (req: Request, res: Response) => {
  try {
    const messages = db.prepare(`
      SELECT c.*, u.name as user_name, u.avatar as user_avatar
      FROM market_live_chat c
...
// POST /api/market/market-live/:id/chat — Send chat message
router.post('/market-live/:id/chat', authMiddleware, (req: Request, res: Response) => {
  try {
    const payload = (req as any).user as JwtPayload;
    const { text } = req.body;
    if (!text || !text.trim()) { res.status(400).json({ error: 'الرسالة فارغة' }); return; }

    const id = crypto.randomUUID();
    db.prepare('INSERT INTO market_live_chat (id, stream_id, user_id, text) VALUES (?, ?, ?, ?)')
      .run(id, req.params.id, payload.userId, text.trim().slice(0, 500));
    db.prepare("UPDATE market_live_streams SET comment_count = comment_count + 1 WHERE id = ?").run(req.params.id);

    const msg = db.prepare(`
      SELECT c.*, u.name as user_name, u.avatar as user_avatar
      FROM market_live_chat c JOIN users u ON c.user_id = u.id
      WHERE c.id = ?
    `).get(id) as any;
...
// POST /api/market/market-live/:id/like — Like stream
router.post('/market-live/:id/like', authMiddleware, (req: Request, res: Response) => {
  try {
    db.prepare('UPDATE market_live_streams SET like_count = like_count + 1 WHERE id = ?').run(req.params.id);
    const stream = db.prepare('SELECT like_count FROM market_live_streams WHERE id = ?').get(req.params.id) as any;
    res.json({ like_count: stream?.like_count || 0 });
  } catch {
    res.status(500).json({ error: 'فشل' });
  }
});

// POST /api/market/market-live/:id/gift — Send gift
router.post('/market-live/:id/gift', authMiddleware, async (req: Request, res: Response) => {
  try {
    const payload = (req as any).user as JwtPayload;
    const { giftType, giftAmount, message } = req.body;

    // Validate gift amount
    const amount = parseFloat(giftAmount) || 0;
    if (amount <= 0) { res.status(400).json({ error: 'قيمة الهدية غير صحيحة' }); return; }
    ...
    if (!user || user.wallet_balance < amount) {
      res.status(400).json({ error: 'رصيد المحفظة غير كافٍ' });
      return;
    }
    ...
    // Add gift to streamer (80% goes to streamer, 20% platform fee)
    const stream = db.prepare('SELECT user_id FROM market_live_streams WHERE id = ?').get(req.params.id) as any;
    if (stream) {
      const streamerShare = amount * 0.8;
      db.prepare('UPDATE users SET wallet_balance = wallet_balance + ? WHERE id = ?').run(streamerShare, stream.user_id);
    }

    // Record gift
    const giftId = crypto.randomUUID();
    db.prepare('INSERT INTO market_live_gifts (id, stream_id, sender_id, gift_type, gift_amount, message) VALUES (?, ?, ?, ?, ?, ?)')
      .run(giftId, req.params.id, payload.userId, giftType || 'heart', amount, (message || '').slice(0, 200));

    // Update stream stats
    db.prepare('UPDATE market_live_streams SET gift_count = gift_count + 1, gift_amount = gift_amount + ? WHERE id = ?').run(amount, req.params.id);

    // Record transaction
    db.prepare(`INSERT INTO transactions (id, user_id, type, amount, description, status)
      VALUES (?, ?, 'gift_sent', ?, ?, 'completed')`)
      .run(crypto.randomUUID(), payload.userId, -amount, `هدية في بث سوق لايف: ${giftType || 'heart'}`);

    res.json({ success: true, message: 'تم إرسال الهدية! 🎁', newBalance: user.wallet_balance - amount });
  } catch (err: any) {
    res.status(500).json({ error: 'فشل إرسال الهدية', details: err.message });
  }
});

// GET /api/market/market-live/:id/gifts — Get gifts for a stream
router.get('/market-live/:id/gifts', optionalAuth, (req: Request, res: Response) => {
  try {
    const gifts = db.prepare(`
      SELECT g.*, u.name as sender_name, u.avatar as sender_avatar
      FROM market_live_gifts g
...
// ─── Ensure market_live tables exist (migration for older DBs) ──────
try {
  db.exec(`
    CREATE TABLE IF NOT EXISTS market_live_streams (
      id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
    ...
    CREATE TABLE IF NOT EXISTS market_live_chat (
      id TEXT PRIMARY KEY,
    ...
    CREATE TABLE IF NOT EXISTS market_live_viewers (
      id TEXT PRIMARY KEY,
      stream_id TEXT NOT NULL,
      user_id TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS market_live_gifts (
      id TEXT PRIMARY KEY,
      stream_id TEXT NOT NULL,
