...
function AppInner() {
  ...
  // 🔧 Chat App Detection: if the URL contains ?app=chat (set by the
  // chat APK's start_url in manifest-chat.webmanifest), redirect to
  // /messages automatically. This makes the chat APK open directly to
  // the conversations list instead of the home page.
  // We do this OUTSIDE the HashRouter so it runs before routing.
  if (typeof window !== 'undefined') {
    const params = new URLSearchParams(window.location.search);
    if (params.get('app') === 'chat') {
      // Set hash to /messages and clean the ?app=chat from URL
      if (!window.location.hash.includes('/messages')) {
        window.location.hash = '/messages';
      }
      // Remove ?app=chat from the URL (keep only the hash)
